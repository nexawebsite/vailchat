...
export default function ChatArea({ chatId }: ChatAreaProps) {
  ...
  const handleCall = (callType: 'audio' | 'video') => {
    ...
    const otherUserId = chatInfo?.participants?.find((p: any) => p._id !== user?.id)?._id;
    if (otherUserId) {
    ...
    <div className="flex flex-col h-full bg-white dark:bg-[#09090b] relative">
      ...
      >
        ...
        <div className="flex items-center gap-6 text-gray-500">
          <button onClick={() => handleCall('audio')} className="hover:text-primary transition-colors"><Phone className="w-5 h-5" /></button>
          <button onClick={() => handleCall('video')} className="hover:text-primary transition-colors"><Video className="w-5 h-5" /></button>
